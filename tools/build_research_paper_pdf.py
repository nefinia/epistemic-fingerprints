...
def build_story():
    ...
    story += [Spacer(1, 15 * mm), P("DIGITAL MINDS  /  AI SAFETY  /  ASSISTANT PERSONA & MODEL IDENTITY", cover_eye), AccentRule(42 * mm, CORAL, 3), Spacer(1, 7 * mm)]
    ...
    contributions = Table([[P("1", cover_eye), P("A separation of stable individuation from effective population diversity.", cover_body), P("2", cover_eye), P("A falsifiable test of whether persona or history produces persistent epistemic differentiation.", cover_body), P("3", cover_eye), P("An open diagnostic for false redundancy, correlated blind spots and tail recovery.", cover_body)]], colWidths=[7 * mm, 47 * mm, 7 * mm, 47 * mm, 7 * mm, 48 * mm])
    ...
    story += [compact_callout("Safety problem", "AI can multiply apparent agents without multiplying independent perspectives. Agreement can then be a correlated failure, not confirmation.", WHITE, CORAL), Spacer(1, 5 * mm)]
    story += [PB("Generative AI changes the scale of intellectual production. A small number of foundation models can now mediate writing, coding, research, design and decision-making for millions of users. This may connect previously isolated knowledge and increase exploration. It may also concentrate epistemic activity around shared representations, defaults and failure modes. The resulting ecosystem could produce vastly more artifacts while covering less conceptual territory."),
              ...
              P("Research objective", PAPER_H2), PB("The pilot asks whether different methods of creating candidate agents from one model produce persistent cross-task traces, whether additional agents expand hypothesis coverage, and whether populations retain consequential minority explanations. These are distinct questions. The study deliberately does not attempt to solve consciousness, personhood or moral status. It asks narrower questions that can be operationalized, falsified and replicated."),
              dense_cards([
    ...
    story += [P("A population-level rather than maximalist view", PAPER_H2), PB("The aim is not maximum diversity. Scientific progress needs convergence: evidence should eliminate hypotheses, and consensus can encode real learning. Unbounded novelty without selection becomes noise. Following work that separates the variety, balance and disparity of a system [9], the more useful question is what kinds and levels of epistemic diversity allow knowledge-producing systems to remain innovative, calibrated and responsive to reality."),
              ...
              P("Epistemic fingerprints", PAPER_H2), PB("An epistemic fingerprint is a repeatable profile over structured reasoning choices, not a hidden essence. It measures <i>individuation</i>: whether an agent is stably distinguishable. It does not establish <i>effective diversity</i>: whether adding that agent expands coverage or contributes an independent failure mode. A population can therefore show strong fingerprints while remaining collectively homogeneous. Shared foundation-model ancestry is the reason to test this rather than assume that agent count equals epistemic sample size.")]
    ...
    story += [compact_callout("Three linked questions", "Do agents become distinguishable, do additional agents expand coverage, and do populations preserve consequential minority hypotheses?", WHITE, CORAL), Spacer(1, 5 * mm)]
    hypotheses = methods_table([
        ...
        [P("RQ1", PAPER_H3), PB("Individuation: does an intervention create a persistent cross-task epistemic trace?", PAPER_SMALL), PB("Between-agent differentiation exceeds repeated variation within agents on multiple features.", PAPER_SMALL)],
        [P("RQ2", PAPER_H3), PB("Effective diversity: how much hypothesis coverage does each added agent contribute?", PAPER_SMALL), PB("A gradual coverage curve indicates marginal gain; rapid saturation indicates redundancy.", PAPER_SMALL)],
        [P("RQ3", PAPER_H3), PB("Tail recovery: do agents preserve consequential minority hypotheses and fail independently?", PAPER_SMALL), PB("Higher critical retention with lower shared-error concentration, interpreted beside accuracy.", PAPER_SMALL)],
    ], [17 * mm, 76 * mm, 79 * mm])
    story += [hypotheses, Spacer(1, 6 * mm), P("Why contrary outcomes remain useful", PAPER_H2), PB("The condition ordering is not assumed. If prompted personas outperform histories, explicit epistemic goals may be more consequential than short formative narratives. If no intervention exceeds baseline variation, that constrains claims that persona or brief context creates an individuated epistemic agent. If diversity rises while accuracy falls, the intervention creates exploration but not necessarily better collective intelligence. A strong safety result may also be counterintuitive: high fingerprints with rapid coverage saturation and shared errors would show distinguishable agents that remain collectively homogeneous."),
              dense_cards([
    ...
    story.append(PageBreak())
    story += heading("06", "Operationalization and measurement", "The study separates stable individuation, effective diversity, task performance and safety-relevant failure independence.")
    measures = methods_table([
        ...
        [P("Critical-hypothesis retention", PAPER_H3), PB("Fraction retaining a designated low-probability but consequential explanation as primary or alternative.", PAPER_SMALL), PB("Tail-recovery probe; not a standalone safety score.", PAPER_SMALL)],
        [P("Marginal epistemic gain", PAPER_H3), PB("Increase in named-hypothesis coverage as agents are added, averaged across exact combinations.", PAPER_SMALL), PB("Rapid saturation suggests false redundancy even when fingerprints are strong.", PAPER_SMALL)],
    ...
    story += [P("Primary analysis", PAPER_H2),
              ...
              paper_bullet("Plot fingerprint, diversity, accuracy, shared error and critical retention side by side; do not collapse them into one score."),
              paper_bullet("Plot exact-combination hypothesis coverage against agent count and report the marginal gain of each added agent."),
    ...
    logic = methods_table([
        ...
        [P("High fingerprint + fast saturation", PAPER_SMALL), PB("Agents are distinguishable but collectively homogeneous: a false-redundancy warning.", PAPER_SMALL), PB("That the agents are useless, conscious or causally identical.", PAPER_SMALL)],
    ], [53 * mm, 64 * mm, 55 * mm])
    ...
    story += [P("Potential contribution to digital minds", PAPER_H2), PB("Digital-mind individuation is often approached through self-reference, preferences, continuity or architecture. Epistemic fingerprints add a population-level behavioral criterion: whether candidate minds contribute independently structured uncertainty and search behavior. The criterion is modest but useful. It can reveal when many apparent agents are functionally redundant for collective inquiry, and when contextual or persona interventions create stable specialization."),
              ...
              dense_cards([
                  ...
                  ("NEXT 7", "Effective sample size", "After validation, develop an Effective Epistemic Sample Size grounded in marginal coverage and failure dependence.", BLUE),
              ...
              PageBreak(), P("Four implications of epistemic diversity", PAPER_H1), PB("The technical pilot is the empirical nucleus of a wider inquiry. These implications motivate future work; they are not claims established by 54 trials."),
              dense_cards([
    ...
    references = [
        ...
        "[12] Irving, G., Christiano, P. &amp; Amodei, D. (2018). AI safety via debate. <link href='https://arxiv.org/abs/1805.00899'>arXiv:1805.00899</link>.",
        "[13] Du, Y. et al. (2024). Improving Factuality and Reasoning in Language Models through Multiagent Debate. <i>ICML 2024</i>. <link href='https://arxiv.org/abs/2305.14325'>arXiv:2305.14325</link>.",
    ]
